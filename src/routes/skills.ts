...
function signaturePayload(familyId: string, versionId: string, version: string, hash: string): string {
  return `${familyId}\n${versionId}\n${version}\n${hash}`
}
...
export function registerSkillRoutes(app: FastifyInstance): void {
  ...
  app.post('/api/v1/skill-submissions', { preHandler: app.authenticate }, async (req, reply) => {
    ...
    const packageKey = await storage.put(buf, 'zip')
    const signature = signServerPayload(
      cfg.masterKey,
      signaturePayload(familyId, versionId, manifest.version, hash)
    )
    ...
    app.audit(req, autoApprove ? 'skill_publish' : 'skill_submit', versionId, {
      familyId,
      scopeId,
  ...
  app.get('/api/v1/skill-submissions/mine', { preHandler: app.authenticate }, async (req, reply) => {
    ...
    const rows = db.prepare(
      `SELECT sv.id AS submissionId, sf.id AS skillId, sf.name, sv.version, sv.state,
              sv.review_note AS reviewNote, sv.created_at AS createdAt,
  ...
  app.get('/api/v1/admin/skill-submissions', {
    ...
    const rows = db.prepare(
      `SELECT sv.id AS submissionId, sf.id AS skillId, sf.name, sf.description,
              ...
              sv.state, sv.created_at AS createdAt, s.id AS scopeId,
    ...
    db.transaction(() => {
      db.prepare(
        `UPDATE skill_versions SET state='approved', reviewer_id=?, review_note=?,
                reviewed_at=?, published_at=? WHERE id=? AND state='pending'`
      ).run(claims.sub, parsed.data.note ?? null, now, now, versionId)
      db.prepare(
        ...
        versionId,
        parsed.data.mandatory === undefined ? null : Number(parsed.data.mandatory),
        parsed.data.allowPersonalOverride === undefined
          ? null
          : Number(parsed.data.allowPersonalOverride),
        now,
        row.familyId
      )
    })()
  ...
  app.get('/api/v1/skills/sync', { preHandler: app.authenticate }, async (req, reply) => {
    ...
    const rows = db.prepare(
      `SELECT sf.id AS skillId, sf.slug, sf.name, sf.description, sv.id AS versionId,
         ...
         JOIN v_effective_scopes s ON s.id = sf.scope_id
         JOIN v_user_scopes us ON us.scope_id = sf.scope_id AND us.user_id = ?
        WHERE sf.state='active' AND sf.updated_at > ? AND sf.updated_at <= ?`
    ).all(claims.sub, cursor, nextCursor) as Record<string, unknown>[]
    ...
    const visibleSkillIds = db.prepare(
      `SELECT sf.id AS skillId
        ...
        ORDER BY sf.id`
    ).all(claims.sub).map((row) => (row as { skillId: string }).skillId)
    return reply.send(ok({
      nextCursor: String(nextCursor),
      leaseUntil: nextCursor + 24 * 60 * 60_000,
      visibleSkillIds,
